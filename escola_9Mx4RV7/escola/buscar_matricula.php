...
// É preciso criar usuario e dar permissão conforme exemplo de cadastrar_disciplina.php
  ...
  if(isset($_POST['prontuario']) and (strlen($_POST['prontuario'])>4) and (strlen($_POST['codigo'])==2)){
    $sql = 'CALL buscar_matricula("'.$_POST['prontuario'].'","'.$_POST['codigo'].'");';
  ...
  <h2>Resultado</h2>
    <table border="1">
    <tr><th>Estudante</th><th>Turma</th><th>Disciplina</th><th>Data da matrícula</th></tr>';
        echo '    <tr><td>'.$row['prontuario'].'</td><td>'.$row['codigo'].'</td><td>'.$row['sigla'].'</td><td>'.$row['data'].'</td></tr>';
        echo '  </table>';
      }
...
?>
  <p>
   <form action="buscar_matricula.php" method="POST">
    ...
    <input type="submit" name="cadastrar" value="Buscar Disciplina" />
   </form>
  </p>
